;(function () {
	...
	var navigationSection = function() {
		...
		// Reflect scrolling in navigation
		var switchIcon = function(section) {
			$("#fh5co-features-2 .fh5co-text").on({
    mouseenter: function () {
       $(this).find('.icon').eq(0).addClass('hide');
			 $(this).find('.icon').eq(1).removeClass('hide');
    },
    mouseleave: function () {
			$(this).find('.icon').eq(0).removeClass('hide');
			$(this).find('.icon').eq(1).addClass('hide');
    }
});
		};
...
$("#contactForm").submit(function(){
        ...
        for (var i in viewArr) {
            contact[viewArr[i].name] = viewArr[i].value;
          }
        $.ajax({
         type     : "POST",
         cache    : false,
...
var $form = $(this);
                                     ...
                                     .done (function(data) { location.href = "http://localhost:4567/contact";})
                                     .fail (function()  { alert("Error ")   ; })
e.preventDefault();
})
